...
import {SearchContainer, SearchBar, ClickToSearchBtn, SearchInput, LoadingButtonContainer, LoadingButton, GreenLoading, SearchBtnContainer, SearchBtn, TitlePreview} from './SearchProductsStyles';
...
const SearchProductsComponent = (props) => {
	...
	useEffect(() => {
		async function fetchProductsBySearch() {
			const {data} = await axios.get(`/api/product/?search=${searchTxt}`)
			props.search(data)
